...
async function main() {
    ...
    await sleep(1000);
    await expect(ethers.BigNumber.from(allowance).eq(ethers.utils.parseEther('400'))).to.be.true;
    ...
    await expect(ethers.BigNumber.from(afterDepositContractBalance).sub(ethers.BigNumber.from(contractBalance))
            .gt(ethers.utils.parseEther('400'))).to.be.true;
    ...
    await expect(ethers.BigNumber.from(controllerBalance).sub(ethers.BigNumber.from(afterDepositControllerBalance))
            .eq(ethers.utils.parseEther('400'))).to.be.true;
    ...
    await expect(ethers.BigNumber.from(afterWithdrawContractBalance).add(ethers.utils.parseEther('300'))
            .gt(ethers.BigNumber.from(afterDepositContractBalance)))
    ...
    await expect(ethers.BigNumber.from(afteWithdrawControllerBalance).sub(ethers.BigNumber.from(afterDepositControllerBalance))
            .eq(ethers.utils.parseEther('300'))).to.be.true;
