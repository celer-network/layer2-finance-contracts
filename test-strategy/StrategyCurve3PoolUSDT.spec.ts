...
describe('StrategyCurve3PoolUSDT', function () {
  async function deploy() {
    ...
    const deployedAddress = process.env.STRATEGY_CURVE_3POOL_USDT;
    if (deployedAddress) {
      ...
      strategy = await strategyCurve3PoolFactory
        ...
        .deploy(
          ...
          process.env.CURVE_USDT as string,
          6,
          2,
    ...
    const usdt = ERC20__factory.connect(process.env.CURVE_USDT as string, deployerSigner);

    return { strategy, usdt, deployerSigner };
  }

  it('should commit, uncommit and optionally harvest', async function () {
    this.timeout(300000);

    const { strategy, usdt, deployerSigner } = await deploy();

    expect(getAddress(await strategy.getAssetAddress())).to.equal(getAddress(usdt.address));

    const strategyBalanceBeforeCommit = await strategy.syncBalance();
    console.log('Strategy USDT balance before commit:', formatUnits(strategyBalanceBeforeCommit, 6));
    ...
    await ensureBalanceAndApproval(
      ...
      deployerSigner,
      strategy.address,
      process.env.CURVE_USDT_FUNDER as string
    );
    const controllerBalanceBeforeCommit = await usdt.balanceOf(deployerSigner.address);
    console.log('Controller USDT balance before commit:', formatUnits(controllerBalanceBeforeCommit, 6));

    console.log('===== Commit 100 USDT =====');
    ...
    expect(strategyBalanceAfterCommit.sub(strategyBalanceBeforeCommit).sub(slippageAmount).lte(commitAmount)).to.be
      .true;
    console.log('Strategy USDT balance after commit:', formatUnits(strategyBalanceAfterCommit, 6));

    const controllerBalanceAfterCommit = await usdt.balanceOf(deployerSigner.address);
    ...
    expect(controllerBalanceBeforeCommit.sub(controllerBalanceAfterCommit).sub(slippageAmount).lte(commitAmount)).to.be
      .true;
    console.log('Controller USDT balance after commit:', formatUnits(controllerBalanceAfterCommit, 6));

    console.log('===== Uncommit 70 USDT =====');
    ...
    expect(strategyBalanceAfterUncommit.add(uncommitAmount).sub(slippageAmount).lte(strategyBalanceAfterCommit)).to.be
      .true;
    console.log('Strategy USDT balance after uncommit:', formatUnits(strategyBalanceAfterUncommit, 6));

    const controllerBalanceAfterUncommit = await usdt.balanceOf(deployerSigner.address);
    ...
    expect(controllerBalanceAfterUncommit.sub(controllerBalanceAfterCommit).sub(slippageAmount).lte(uncommitAmount)).to
      .be.true;
    console.log('Controller USDT balance after uncommit:', formatUnits(controllerBalanceAfterUncommit, 6));
    ...
    try {
      ...
      if (harvestGas.lte(1000000)) {
        ...
        console.log('Strategy USDT balance after harvest:', formatUnits(strategyBalanceAfterHarvest, 6));
      }
    } catch (e) {
